...
<head>
  ...
  <title>This Care VIDEO REFERENCE</title>
...
<body>
  <header style="margin-bottom: 20px;">
    <img class="logo" src="${cp}/RTCMultiConnection-master/demos/logo.png" alt="RTCMultiConnection" /> <span class="logo-text">This Care VIDEO REFERENCE</span>

    <div style="float: right;margin-top: 15px;">
      <button class="btn btn-primary" data-toggle="modal" data-target="#startRoomModel">방 만들기</button>
      <button id="btn-show-join-hidden-room" class="top-span btn btn-secondary" data-toggle="modal" data-target="#joinRoomModel">참여하기</button>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="startRoomModelLabel">방 만들기</h5>
        ...
        <div class="modal-body" style="padding-bottom: 0;">
          <form>
            <div class="form-group">
              <p>
                <label for="txt-roomid" class="col-form-label">방 제목 입력:</label>
                <input type="text" class="form-control" id="txt-roomid">
              </p>

              <p>
                <label for="txt-user-name" class="col-form-label">이름 입력:</label>
                <input type="text" class="form-control" id="txt-user-name">
              </p>

              <p style="display: none;">
                <label for="txt-room-password" class="col-form-label">패스워드 입력:</label>
                <input type="text" class="form-control" id="txt-room-password">
              </p>

              <a href="" style="float: right; margin-bottom: 10px; font-size: 14px;" onclick="event.preventDefault();$('.more-options').show();$(this).hide();">옵션 더보기</a>

              <p class="more-options" style="margin-bottom: 0; display: none;">
                <label class="col-form-label"><input type="checkbox" id="chk-room-password"> 패스워드를 설정하시겠습니까?</label>
              </p>

              <p class="more-options" style="margin-top: 0; margin-bottom: 0; display: none;">
                <label class="col-form-label"><input type="checkbox" id="chk-hidden-room"> 방을 숨기겠습니까? (리스트에 출력되지 않는 방)</label>
              </p>
            </div>
          </form>
        </div>
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" id="btn-create-room">만들기</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="joinRoomModel" tabindex="-1" role="dialog" aria-labelledby="joinRoomModelLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="joinRoomModelLabel">참여하기</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form>
            <div class="form-group">
              <p>
                <label for="txt-roomid-hidden" class="col-form-label">방 제목을 입력하세요:</label>
                <input type="text" class="form-control" id="txt-roomid-hidden">
              </p>

              <p>
                <label for="txt-user-name-hidden" class="col-form-label">이름을 입력하세요:</label>
                <input type="text" class="form-control" id="txt-user-name-hidden">
              </p>

              <p style="display: none;">
                <label for="txt-room-password-hidden" class="col-form-label">패스워드를 입력하세요:</label>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" id="btn-join-hidden-room">대화 참여</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <footer style="text-align: center;">This Care VIDEO REFERENCE | How Care</footer>
...
$('#btn-join-hidden-room').click(function() {
    ...
    if($('#txt-room-password-hidden').parent().css('display') !== 'none') {
      ...
      if (!roomPassword || !roomPassword.replace(/ /g, '').length) {
          alertBox('비밀번호를 입력해주세요', '비밀번호가 필요합니다');
          return;
      }
...
function openCanvasDesigner() {
    ...
    //location.href뺐음
    var href = '/RTCMultiConnection-master/demos/dashboard/canvas-designer.html?open=' + connection.isInitiator + '&sessionid=' + connection.sessionid + '&publicRoomIdentifier=' + connection.publicRoomIdentifier + '&userFullName=' + connection.extra.userFullName;
